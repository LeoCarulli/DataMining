...
from prompt import template, prompt_7b_2
...
prompts = [
    ...
    "show the project name of code number 32",
    "distinct project type ordered alfabetical",
    ]
...
expected_outputs = [
    ...
    [('SVP_Anon_3',), ('SVP_Anon_5',), ('SVP_Anon_7',), ('SVP_Anon_1',), ('SVP_Anon_6',), ('SVP_Anon_2',), ('SVP_Anon_4',)],
    [(105,)],
    [(52,)],
    [('target enterprise web services',)],
    [('Account Managers (Payer)',), ('Clinical Educators',), ('Clinical Educators/On Call Nurses',), ('Clinical Educators/On Call Nurses - MedA',), ('Clinical Educators/On call Nurses',), ('Engagement Center',), ('Fleet Only',), ('General Kinetic',), ('Medical Science Liaisons',), ('Ops Only',), ('REMS Engagement Center',), ('Recruiting Only',), ('Reimbursement/ Access',), ('Reimbursement/Access',), ('Sales Representatives',), (None,)],
...
def test_model(prompts, expected_outputs, db_path, chain, max_attempts=5):
    ...
    for prompt, expected_output in zip(prompts, expected_outputs):
        ...
        print(prompt)
        query, _ = handle_conversation(prompt, "", db_path, max_attempts=max_attempts, chain=chain, json_path=json_path)
        if "Unable to generate a valid SQL query" in query:
            ...
            if output == expected_output:
                responses_correct += 1
            elif soft_equal(output, expected_output):
                responses_correct += 0.5
                print("Aparecen los valores esperados en la respuesta pero hay columnas extras que no fueron solicitadas")
            else:
                print("Incorrect test")
                print(f"Excepted output: {expected_output}")
...
ollama_params_baseline_better_model = {"model": "pxlksr/defog_sqlcoder-7b-2:F16"}
...
model_parameters = [
    ...
    {"name": "more attemps", "ollamaparams": ollama_params_baseline_better_model, "test_params": {"max_attempts": 5}},
